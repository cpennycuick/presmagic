define([
		...
		app.db.transaction('rw', app.db.frame, function () {
			...
			app.db.frame
				...
				.toArray(function (dbframes) {
					for (var j = 0; j < dbframes.length; j++) {
	...
	c.prototype._shiftFrame = function (frameIdx, direction) {
		...
		switch (direction) {
			case 'left':
				if (frameIdx == 0) {
					return false; //already left most position
				}
				newIndex = frameIdx - 1;
				break;
			case 'right':
				if (frameIdx == frameslength - 1){
					return false; //already right most position
				}
	...
	c.prototype._openEditorDialog = function (frameIdx) {
		...
		app.loadPanel('app/panel', $('#Content'), this, opts)
			.then(function (panel) {
				panel.run();
			}, function (error) {
